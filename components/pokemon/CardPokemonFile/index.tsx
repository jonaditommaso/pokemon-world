...
const CardPokemonFile = ({
  ...
  const initial_state_to_review = ranking.find(
    (poke: any) => poke.pokemon === pokemonData?.name
  );
  ...
  const pokemonColor =
    colorsByType[pokemonData?.types[0].type.name as keyof typeof colorsByType];
  ...
  return (
    <div className={styles.pokemonFile} style={{ margin: modal ? 0 : "" }}>
            ...
            ({ type }: { type: { name: string; url: string } }) => (
              <div key={type.name} className={styles["pokemon-type-container"]}>
                ...
                <div
                  className={clsx(
                    styles["sliding-div"],
                    !openTypes.includes(type.name) ? styles.pressed : ""
                  )}
                >
                  Type:{" "}
                  <span
                    style={{
                      color:
                        colorsByType[type.name as keyof typeof colorsByType],
                    }}
                  >
                    {type.name}
                  </span>
      ...
      <Card
        className={styles.searchPokemon__card}
        style={{ boxShadow: modal ? "none" : "" }}
      >
            ...
            <div>
              <div
                ...
                style={{
                  backgroundImage: modal
                    ? ""
                    : generateGradient(pokemonColor, "bottom"),
                }}
              >
                ...
                <CardContent sx={{ p: "0 !important" }}>
                  <Typography
                    className={styles.pokemonFile__title}
                    variant='h5'
                  >
              ...
              {!modal && (
                <CardContent>
                  ...
                  <Typography
                    align='center'
                    className={styles["pokemon-file-description"]}
                  >
                    {DESCRIPTION}
                  </Typography>
                </CardContent>
            ...
            >
              <span
                className={clsx(
                  ...
                  iconActions[option.button as IconName]?.disableButton &&
                    styles["disabled-pokemon-option"]
                )}
                style={{
                  backgroundColor: option.backgroundColor,
                  cursor: iconActions[option.button as IconName]?.disableButton
                    ? "default"
                    : "pointer",
...
export default connect(mapStateToProps, { pauseMusic, playMusic })(
  CardPokemonFile
);
